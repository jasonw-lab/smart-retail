...
- ルーティングは ADR-009 に従い、`app/[locale]/(auth)` と `app/[locale]/(dashboard)` を正とする。
- API Route は `app/api/auth/{login,logout,refresh,me,captcha,csrf,ws-ticket}`、`app/api/proxy/[...path]`、`app/api/ws/connect`、`app/api/health`、`app/api/report` を含む。
- `data-testid` 定数は `lib/testing/testids.ts` を正とし、`e2e/testids.ts` は互換用 re-export とする。本番コードから `e2e/` への import は禁止する。
- ESLint では `@/e2e/testids`、`next/link`、`next/navigation` の `useRouter` / `usePathname` を禁止し、共有 testids と `@/i18n/navigation` へ誘導する。
- `lib/api/server.ts` は `server-only` を import し、Server Component 専用境界をビルド時にも明示する。

## 変更履歴

| 日付       | 内容                                                   |
| ---------- | ------------------------------------------------------ |
| 2026-07-10 | i18n構成、API Route一覧、testids配置、ESLint境界を追記 |
