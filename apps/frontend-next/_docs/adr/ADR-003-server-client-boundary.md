...
- 一覧ページの標準は、`page.tsx` が Server Component として初期データを取得し、Client Component が TanStack Query で後続操作を行う二層構成とする。
- 検索条件・ページ番号は URL を single source of truth とする。Client 内の `useState(initialParams)` はフォーム操作中の一時状態に限定し、検索・ページ変更時は URL を更新する。
- API障害は空配列に変換せず、Server Component では `error.tsx` へ委ねる。Client 側の再取得失敗ではインラインエラーを表示し、初期データを表示し続ける場合も stale であることを明示する。
- 認証付き Server fetch のキャッシュ方針は ADR-010 を参照する。

## 変更履歴

| 日付       | 内容                                          |
| ---------- | --------------------------------------------- |
| 2026-07-10 | 一覧ページ標準、URL状態、エラー表示方針を追記 |
