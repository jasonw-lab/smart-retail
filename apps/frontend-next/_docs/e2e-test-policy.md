...
```
e2e/
  testids.ts          # lib/testing/testids.ts の互換 re-export
lib/
  testing/
    testids.ts        # data-testid 定数の正本
```
...
| API応答不正    | `expect(received).toEqual(expected)` |    ❌    | 仕様判断が必要なため人間にエスカレート  |
...
3. 失敗パターンを分類
4. 自動修正可能なら修正ブランチで実装修正
5. 再実行（最大3回）
6. 修正diffをレビュー記録へ追記
7. 修正不可 or 3回失敗 → 人間にエスカレート
```
...
```bash
# 自律修正モード
claude -p "e2e/results.json を読み、失敗テストを調査せよ。アサーション期待値、スナップショット、テストのskip/削除、mock-server.tsの仕様レスポンス変更は禁止。必要と判断した場合は修正せず理由を報告せよ。修正後は pnpm test:e2e で検証。"

# 単一テスト修正
claude -p "e2e/specs/auth.spec.ts:31 の失敗を調査せよ。アサーション期待値、スナップショット、テストのskip/削除、mock-server.tsの仕様レスポンス変更は禁止。スクショ: e2e/test-results/..."
```
...
5. mock server のレスポンス仕様変更が必要に見える場合
