...
- 現行実装は `/api/auth/ws-ticket` で短TTLチケットを発行し、`/api/ws/connect` でチケットを accessToken に交換して STOMP `connectHeaders` に設定する。
- この方式はアクセストークンを永続保存しないが、最終的に JavaScript メモリ上へ accessToken が載るため、「生のaccessTokenをJavaScriptへ露出しない」という表現は採用理由として使わない。リスク低減は「短時間・メモリ上の利用に限定する」範囲に留まる。
- accessToken をブラウザへ返さない完全な方式は、Backend が STOMP CONNECT 時にチケットを検証できるようにするサーバー間交換方式であり、Backend 改修が必要な将来課題とする。
- alert store は重複ID受信時に unreadCount がドリフトしないよう、保持配列から unreadCount を再計算する。

## 変更履歴

| 日付       | 内容                                                       |
| ---------- | ---------------------------------------------------------- |
| 2026-07-10 | STOMPチケット方式の実効リスクと alert-store 重複排除を追記 |
