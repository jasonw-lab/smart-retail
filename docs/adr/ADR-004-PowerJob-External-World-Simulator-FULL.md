...
# QA
2026-01-28 23:15 , Antigravity　異常系（決済端末停止）の判定ロジックについて：Heartbeat正常かつ最終決済日時が閾値超えで判定していますが、夜間や閑散期など「単に売上がない」場合と「故障」をどう区別しますか？Heartbeatペイロードにデバイス健全性（リーダー接続状態など）を含める検討は必要ありませんか？

# Proposal
2026-01-28 23:20 , Antigravity　**Heartbeatペイロードの拡張による状態監視の精緻化**:
Heartbeat APIのペイロードに `deviceStats` フィールドを追加し、エッジ側の自己診断結果（リーダー接続、プリンター用紙、ネットワーク安定度など）を送信する仕様を提案します。
これにより、「Heartbeatはあるが決済機能が死んでいる」状態を即座に検知でき、売上ベースの推測（不確実）に頼らない確実な監視が可能になります。PowerJob側では `simulateDeviceFailure` ジョブを作成し、このステータス異常をシミュレートさせます。

# Answer
2026-01-28 23:45 , wangjw　**QA（決済端末停止の判定ロジック）への回答**:
ご指摘ありがとうございます。おっしゃる通り、「売上がない時間帯」と「故障」の区別は重要な課題です。
