...
### 選択肢4: Playwright + Standalone HTTP Mock Server（採用）

- PlaywrightでE2E
- `BACKEND_URL` を standalone mock server へ向ける
- Server Component の Backend 直接 fetch も同じモックで検証可能

### 選択肢5: Storybook + Chromatic
...
**Playwright + Standalone HTTP Mock Server（選択肢4）を採用する。**

当初候補の Playwright + MSW は、Browser の Service Worker では Server Component が Next.js サーバープロセス内で実行する `fetchFromBackend()` を捕捉できないため却下する。Backend API の代替は `BACKEND_URL` を `e2e/mocks/mock-server.ts` に向ける方式を正とする。
...
2. **Standalone Mock ServerによるRSC対応**
   - Server Component の Backend 直接 fetch を捕捉できる
   - Client Component の `/api/proxy/*` 経由リクエストも同じ Backend URL で検証できる
   - E2E 環境と本番環境の API 契約差分を `mock-server.ts` に集約できる
...
export const productHandlers = [
  ...
  http.get('/api/proxy/retail/products/page', ({ request }) => {
    ...
    const filtered = mockProducts.filter((p) => p.productName.includes(search));
...
- standalone mock server と実APIの乖離リスク
- WebSocketモックの複雑性
- mock server のAPI契約更新漏れ
...
| フェーズ | 内容                                             | 見積 |
| -------- | ------------------------------------------------ | ---- |
| Phase 1  | Playwright + Standalone Mock Server セットアップ | -    |
| Phase 2  | 認証フローテスト                                 | -    |
| Phase 3  | 商品CRUDテスト                                   | -    |
