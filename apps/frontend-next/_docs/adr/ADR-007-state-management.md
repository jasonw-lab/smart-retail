...
export const productApiServer = {
  ...
  getById: (id: number) => fetchFromBackend<Product>(`retail/products/${id}`),
};
```
...
- 採用バージョンは `package.json` / lockfile を正とする。2026-07-10 時点の実装は TanStack Query v5、Zustand v5、React Hook Form v7 を使用する。
- 検索・ページネーションなど共有可能な画面状態は URL 状態を正とする。`useSearchParams` は読み取り、画面遷移とURL更新は ADR-009 に従い `@/i18n/navigation` を使う。
- Zustand の永続化を使う場合は hydration mismatch を避けるため、SSR初期値との差分がレイアウトを大きく変えない状態に限定する。影響が大きい状態は Cookie 永続化または `skipHydration` + 手動 rehydrate を検討する。
- アラートの unreadCount は保持配列から導出し、重複受信や保持上限によるドリフトを避ける。

## 変更履歴

| 日付       | 内容                                                              |
| ---------- | ----------------------------------------------------------------- |
| 2026-07-10 | 採用バージョン、URL状態、Zustand hydration、alert-store方針を追記 |
