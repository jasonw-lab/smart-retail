...
### ハイブリッド E2E 運用戦略（案A: モックE2E × 実結合Liveスモーク）

「モックテストでバグを見逃したなら、すべてのE2Eテストを本物のバックエンドとMySQLに繋げば良いのではないか？」という疑問が生まれます。しかし、全E2Eを実機接続すると以下の**3大問題**に直面します。

1. **データ破壊と状態汚染（Side Effects）**:
   新規在庫登録、在庫廃棄、削除などのCRUD操作を実行すると、実DBのレコードが書き換わります。2回目以降のテスト実行時に期待する初期データが破壊され、テストが連鎖的にFlaky化します。
2. **実行速度とCI実行コスト（Speed & Cost）**:
   Java/Spring BootやMySQLコンテナの起動待ち、トランザクションの永続化により、テスト実行時間が数倍〜十数倍に肥大化。PR作成ごとの高速フィードバックが失われます。
3. **境界値・異常系の再現性（Edge Cases）**:
   「通信タイムアウト」「500サーバー内部エラー」「極端な品切れ（0件表示）」など、実DBでは意図的に作りにくい異常系やエッジケースを検証できません。

#### モックE2E vs 実結合Liveスモークの役割分担マトリクス

本プロジェクトで採用する「案A（ハイブリッド運用）」では、以下の通り明確に責務を分離します。

| 比較項目 | 通常 E2E テスト (`pnpm test:e2e`) | 実結合スモークテスト (`pnpm test:smoke`) |
| :--- | :--- | :--- |
| **接続先** | **E2E用モックサーバー** (`mock-server.ts:8095`) | **実バックエンド (:8080) ＋ 実 MySQL / Redis** |
| **バックエンド/DB** | 起動不要（フロント単独で即実行可能） | 起動必須（ローカルDockerまたはステージング実機） |
| **検証の主眼** | **UI操作・CRUD・状態遷移の完全網羅**<br>（ボタン・リンク・ダイアログ・バリデーション全18件） | **実API契約・実DBデータとの結合整合性**<br>（画面クラッシュゼロ・実DB検索絞り込み除外） |
| **実行速度** | 約 30 秒（UI・シナリオ全網羅） | **約 4 秒**（主要画面超高速巡回） |
| **モック運用の原則** | **バックエンド未実装パラメータを代行しない**<br>実機の契約挙動（全件返すなら全件返す）に忠実にする | 本物のAPIレスポンス・本物のCookie/JWT認証を直接検証 |

```mermaid
flowchart LR
  subgraph LocalDev["日常の開発サイクル (フロント完結)"]
    Code["コード実装 / 修正"] --> TypeCheck["型検査 (pnpm typecheck)"]
    TypeCheck --> MockE2E["モックE2E (pnpm test:e2e)<br/>全UI・CRUD・ダイアログ網羅"]
  end

  subgraph PreCommit["コミット前・デプロイ前 (実機結合)"]
    MockE2E --> LiveSmoke["実結合スモーク (pnpm test:smoke)<br/>:8080 実DB検索・除外・クラッシュ検証"]
    LiveSmoke --> Commit["Git Commit & Push"]
  end

  subgraph DeployPhase["パイプライン / デプロイ"]
    Commit --> CI["CI パイプライン (モックE2E高速実行)"]
    CI --> Staging["ステージング環境 (Liveスモーク実行)"]
  end
```
