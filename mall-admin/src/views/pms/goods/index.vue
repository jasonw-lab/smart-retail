...
const { loading, ids, queryParams, goodsList, categoryOptions, goodDetail, total, dialogVisible } =
  toRefs(state);

function handleQuery() {
  state.loading = true;
  GoodsAPI.getSpuPage(state.queryParams)
    .then((data) => {
    ...
    <div class="search-container">
      <el-form ref="queryFormRef" :model="queryParams" :inline="true">
        <el-form-item label="关键字">
          <el-input v-model="queryParams.name" placeholder="商品名称" clearable />
        </el-form-item>
    ...
    <el-card>
      ...
      >
        ...
        <el-table-column label="图片">
          <template #default="scope">
            <el-popover placement="right" :width="400" trigger="hover">
              ...
              <template #reference>
                <img :src="scope.row.picUrl" style="max-width: 60px; max-height: 60px" />
              </template>
            </el-popover>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="200">
          <template #default="scope">
            <el-button type="success" link @click.stop="handleGoodsView(scope.row.detail)">
              查看详情
            </el-button>
