...
- middleware で `access_token` が無く `refresh_token` が残っている場合、Backend の refresh-token API を呼び出して Cookie を再発行し、同一URLへリダイレクトしてSSR経路のセッション継続性を確保する。
- Client Component の 401 リトライは `lib/api/client.ts` の single-flight refresh を正とし、Route Handler proxy はリフレッシュ責務を持たない。
- 認証付き Server fetch は ADR-010 に従い、暗黙キャッシュせず `cache: 'no-store'` を既定とする。
- CSRF は Double Submit Cookie (`/api/auth/csrf` + `x-csrf-token`) を正とし、状態変更リクエストは proxy で検証する。
- ローカルモック認証は `NODE_ENV !== 'production' && ENABLE_LOCAL_AUTH_MOCK === 'true'` の場合のみ有効とし、本番では起動時に fail-fast する。

## 変更履歴

| 日付       | 内容                                                                      |
| ---------- | ------------------------------------------------------------------------- |
| 2026-07-10 | SSR refresh、client single-flight、CSRF、no-store、モック認証ガードを追記 |
